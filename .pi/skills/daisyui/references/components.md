...
This file is the official daisyUI LLM reference (sourced from <https://daisyui.com/llms.txt>, daisyUI 5.5.x).

Each component section includes:

- Class names by type (component, part, style, color, size, modifier, placement)
- HTML syntax template with `{PLACEHOLDER}` variables
- Usage rules

**Component index:**
Actions: `button` `dropdown` `fab` `modal` `swap` `theme-controller`
Data display: `accordion` `avatar` `badge` `card` `carousel` `chat` `collapse` `countdown` `diff` `hover-3d` `hover-gallery` `kbd` `list` `stat` `status` `table` `text-rotate` `timeline`
...
Data input: `calendar` `checkbox` `fieldset` `file-input` `filter` `input` `join` `label` `radio` `range` `rating` `select` `textarea` `toggle` `validator`
